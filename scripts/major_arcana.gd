...
const arcana_penalty_effect: Array[String] = ["For this round, your hand will be maxed at 3 cards, but, for each turn you will have “infinite” redraw of your entire hand, meaning you discard your hand and draw new cards. If you don’t have any more cards in your hand and deck, the round ends immediately.", 
...
const arcana_bonus_effect: Array[String] = ["No effects", 
...
func majorAppliedToPoint(majorPenalty:int, cardPoint: float, card: Node, cardPlayed: int, lastCard: Node, cardToPlay: int) -> float:
	...
	match majorPenalty:
		...
		10:
			...
			if (!randomEffectDone and cardToPlay == cardPlayed):
				return 1
			if (!randomEffectDone ):
...
func canPlayCard (majorPenalty: int, card: Node, lastCard: Node, cardPlayed: int) -> bool:
	...
	match majorPenalty:
		1:
			...
			if (lastCardNum != 0 and cardNum%2 == lastCardNum%2):
				print("You played two cards of the same parity")
				return false
		5:
			if(cardPlayed > 0):
				if (increasing == 1):
					...
					if (increasing == 2):
						...
						if (cardNum >= lastCardNum):
							...
							increasing = 1 #for decreassing needed
		_:
			return true
	
	return true

#Arcana that change the score
